...
syms theta(t) psi(t) phi(t) g m h w d l
...
omega = [ diff(phi(t), t)*cos(psi(t)) + diff(theta(t), t)*sin(phi(t))*sin(psi(t)); 
    -diff(phi(t), t)*sin(psi(t)) + diff(theta(t), t) *sin(phi(t))*cos(psi(t)); 
    diff(theta(t), t) *cos(phi(t)) + diff(psi(t), t)  ]
...
% For psi:
eq2 = simplify(diff(diff(K, diff(psi(t), t) ), t)   + diff(UE, psi(t))   == 0)
% For phi:
eq3 = simplify(diff(diff(K, diff(phi(t), t)), t)    + diff(UE, phi(t))   == 0)


vars = [theta(t); diff(theta(t), t); phi(t); diff(phi(t), t); psi(t); diff(psi(t), t)];
...
plot(t, Y(:,5), 'g', 'DisplayName', '\phi Nutation');
plot(t, Y(:,3), 'b', 'DisplayName', '\psi Spin');
legend;
xlabel('Time');
ylabel('Values');
