...
l_val = 30;      % pendulum length
g_val = 9.81;    % gravitational acceleration
% Y0: [theta3, theta3dot, theta1, theta1dot, theta2, theta2dot]
Y0 = [0, 0, 0, 0, 0.5, 0];  % example initial conditions
tic
...
%% Compute Cartesian coordinates for the bob
% For the bob position we interpret:
%   R1 = Y(:,3) as the azimuth,
%   R2   = Y(:,5) as the polar angle.
L = abs(l_val);
x = L * sin(Y(:,5)) .* cos(Y(:,3));
y = L * sin(Y(:,5)) .* sin(Y(:,3));
z = -L * cos(Y(:,5));
